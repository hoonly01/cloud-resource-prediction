...
def vali(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric, rag_retriever=None):
    n_features = args.enc_in
    total_loss = []
    total_mae_loss = []
    per_feature_mae = [[] for _ in range(n_features)]
    per_feature_mse = [[] for _ in range(n_features)]
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(vali_loader)):
            ...
            for c in range(n_features):
                per_feature_mae[c].append(mae_metric(pred[..., c], true[..., c]).item())
                per_feature_mse[c].append(criterion(pred[..., c], true[..., c]).item())

    total_loss = np.average(total_loss)
    total_mae_loss = np.average(total_mae_loss)
    feat_names = FEATURE_NAMES[:n_features]
    per_feat_mae = {feat_names[c]: float(np.average(per_feature_mae[c])) for c in range(n_features)}
    per_feat_mse = {feat_names[c]: float(np.average(per_feature_mse[c])) for c in range(n_features)}

    model.train()
    return total_loss, total_mae_loss, per_feat_mae, per_feat_mse
